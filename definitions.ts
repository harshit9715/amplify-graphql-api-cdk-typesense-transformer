...
const getScalarFilterInputType = (
  condition: string,
  type: string
): TypeNode => {
  switch (condition) {
...
export const makeSearchableXFilterInputObject = (
  ...
  if (!obj.fields) {
    throw new InvalidDirectiveError(
      "Models decorated with @searchable require fields defined within them."
    );
  }
    ...
    .map(
      (field: FieldDefinitionNode) =>
        ({
          ...
          name: field.name,
          type: makeNamedType(
            SearchableResourceIDs.SearchableFilterInputTypeName(
              getBaseType(field.type)
            )
          ),
          // TODO: Service does not support new style descriptions so wait.
          // description: field.description,
      ...
      .map(
        (field: FieldDefinitionNode) =>
          ({
            ...
            name: field.name,
            type: makeNamedType(
              SearchableResourceIDs.SearchableFilterInputTypeName("String")
            ),
            // TODO: Service does not support new style descriptions so wait.
            // description: field.description,
            directives: [],
...
export const makeSearchableSortDirectionEnumObject =
  (): EnumTypeDefinitionNode => {
...
export const makeSearchableXSortableFieldsEnumObject = (
  obj: ObjectTypeDefinitionNode
): EnumTypeDefinitionNode => {
...
export const makeSearchableXAggregateFieldEnumObject = (
  obj: ObjectTypeDefinitionNode,
  document: DocumentNode
): EnumTypeDefinitionNode => {
...
export const makeSearchableXSortInputObject = (
  obj: ObjectTypeDefinitionNode
): InputObjectTypeDefinitionNode => {
...
export const makeSearchableAggregateTypeEnumObject =
  (): EnumTypeDefinitionNode => {
...
export const makeSearchableXAggregationInputObject = (
  obj: ObjectTypeDefinitionNode
): InputObjectTypeDefinitionNode => {
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        kind: Kind.INPUT_VALUE_DEFINITION,
        name: { kind: "Name", value: "field" },
        type: makeNonNullType(
          makeNamedType(`Searchable${obj.name.value}AggregateField`)
        ),
        directives: [],
      },
    ],
