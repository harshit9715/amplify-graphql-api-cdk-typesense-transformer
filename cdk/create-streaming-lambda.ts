...
export const createLambda = (
  ...
  const { OpenSearchStreamingLambdaFunctionLogicalID } =
    ResourceConstants.RESOURCES;

  const enviroment: { [key: string]: string } = {
    TYPESENSE_API_KEY: parameterMap.get(TYPESENSE_PARAMS.typesenseApiKey)!
      .valueAsString,
    TYPESENSE_HOST: parameterMap.get(TYPESENSE_PARAMS.typesenseHost)!
      .valueAsString,
    TYPESENSE_PORT: parameterMap.get(TYPESENSE_PARAMS.typesensePort)!
      .valueAsString,
    TYPESENSE_PROTOCOL: parameterMap.get(TYPESENSE_PARAMS.typesenseProtocol)!
      .valueAsString,
    TYPESENSE_FIELDS_MAP: parameterMap.get(TYPESENSE_PARAMS.typesenseFieldsMap)!
      .valueAsString,
  };

  const codeZipPath = path.join(__dirname, '..', 'code.zip');
...
export const createLambdaRole = (
  ...
  const { OpenSearchStreamingLambdaIAMRoleLogicalID } =
    ResourceConstants.RESOURCES;
  ...
  const role = new Role(stack, OpenSearchStreamingLambdaIAMRoleLogicalID, {
    ...
    roleName: context.resourceHelper.generateIAMRoleName(
      parameterMap.get(OpenSearchStreamingIAMRoleName)?.valueAsString ?? ""
    ),
  });
...
export const createEventSourceMapping = (
  ...
  return new EventSourceMapping(
    stack,
    SearchableResourceIDs.SearchableEventSourceMappingID(type),
    {
